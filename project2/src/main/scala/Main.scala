...
object Main extends App {
	...
	val playerOne = new HumanPlayer ("Player 2", Player2)
	...
	private def draw(grid: Grid, playerOne: Player, playerTwo: Player): Unit = {
		println(s"""${playerOne.name} : ${playerOne.disc.asciiRepresentation}, ${playerTwo.name} : ${playerTwo.disc.asciiRepresentation}, No value: o""")
